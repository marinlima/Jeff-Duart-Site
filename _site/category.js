...
catMenuBtn?.addEventListener('mouseleave', () => {
  catCloseTimeout = setTimeout(() => catMenu.classList.remove('open'), 400);
});
...
lbImg.addEventListener('click', e => {
  ...
  if (scale === 1) {
    scale = 2.5;
    posX -= (offsetX - rect.width / 2);
    posY -= (offsetY - rect.height / 2);
  } else {
    resetZoom(); return;
  }
  applyTransform();
...
lbImg.addEventListener('touchmove', e => {
  if (e.touches.length === 2) {
    scale = Math.min(Math.max(1, initialScale * (getDistance(e.touches) / initialDistance)), 4);
    applyTransform();
  }
...
lbImg.addEventListener('touchend', () => { isDragging = false; initialDistance = null; });
...
hamburgerBtn?.addEventListener('mouseleave', () => {
  dropCloseTimeout = setTimeout(() => dropdown.classList.remove('open'), 400);
});
